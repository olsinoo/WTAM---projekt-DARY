      ...
      <div className="nav-section">
        ...
        <div className="nav-topings-Name">
          ...
          <Link className="nav-my-basket" to={"/recipes-basket"}>
              <div>{ !price ? "" : Number(price).toFixed(2) + "eur"}  {!lim ?  <FontAwesomeIcon icon={faBasketShopping}/> : price < lim ? <FontAwesomeIcon icon={faBasketShopping} color={"lightgreen"}/> : <FontAwesomeIcon icon={faBasketShopping} color={"red"}/>}</div>
          </Link>
        </div>
      </div>
    </nav>
